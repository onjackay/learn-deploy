...
model = torchvision.models.mobilenet.mobilenet_v2(weights="IMAGENET1K_V2")
model = model.to(DEVICE)

# export unquantized graph.
torch.onnx.export(model, torch.rand(size=[BATCHSIZE] + INPUT_SHAPE).to(DEVICE),
                    'Output/unquantized(onnx).onnx', verbose=False)
